import { CHECHAK_RANK, CHECHAK_SUIT } from './constants.js';

/**
 * Represents a playing card.
 * @typedef {import('./constants.js').Suit} Suit
 * @typedef {import('./constants.js').Rank} Rank
 * @typedef {object} Card
 * @property {Suit} suit
 * @property {Rank} rank
 */
 ...
 */
const isChechak = (card) => card && card.suit === CHECHAK_SUIT && card.rank === CHECHAK_RANK;
...
export const calculateHandScore = (hand) => {
  ...
  const isThreeOfAKind = hand[0].rank === hand[1].rank && hand[1].rank === hand[2].rank;
  if (isThreeOfAKind) {
